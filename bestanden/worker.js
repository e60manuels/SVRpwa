...
async function handleRequest(request) {
  ...
  try {
    ...
    if (url.pathname === '/login' && request.method === 'POST') {
      ...
      console.log(`[${requestId}] Login attempt received.`);
    ...
    if (cookieParts.length > 0) {
        ...
        console.log(`[${requestId}] Final Constructed Cookie header includes: ${uniqueCookieParts.map(p => p.split('=')[0]).join(', ')}`);
    }
    ...
    // svr.nl returns HTTP 200 for BOTH a real object detail page AND the generic
    // "session expired / please log in" shell page, so the HTTP status alone
    // cannot be used to distinguish them. Real object detail pages include the
    // Sentry monitoring script; the generic/login-fallback shell page does not.
    // Without this check, an expired session silently renders as a confusing
    // "Mail met link is verstuurd!" page instead of prompting re-login.
    if (targetPath.startsWith('/object/') && !responseText.includes('sentry-cdn')) {
        console.warn(`[${requestId}] Detected auth-fallback/login page instead of object detail. Session likely invalid or expired.`);
        return new Response(JSON.stringify({
            error: 'session_invalid',
            message: 'SVR session appears expired or invalid. Please log in again.'
        }), {
            status: 401,
            headers: { 'Content-Type': 'application/json', ...corsHeaders }
        });
    ...
    for (const [key, value] of svrResponse.headers.entries()) {
      if (key.toLowerCase() === 'set-cookie') {
        ...
        const cookieNameMatch = transformedCookie.match(/^([^=]+)=/);
        console.log(`[${requestId}] Transformed and forwarded Set-Cookie from SVR to PWA (proxied), name: ${cookieNameMatch ? cookieNameMatch[1] : 'unknown'}`);
      }
    }
