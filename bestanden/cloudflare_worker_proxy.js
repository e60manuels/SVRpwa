...
async function handleRequest(request) {
  ...
  const corsHeaders = {
    'Access-Control-Allow-Origin': allowOrigin,
    'Access-Control-Allow-Methods': 'GET, HEAD, POST, OPTIONS',
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, Cookie, X-Requested-With', // Ensure this is also sent on non-OPTIONS
    'Access-Control-Allow-Credentials': 'true',
  }
  ...
  try {
    ...
    // Map worker path directly to SVR path
    // e.g. https://worker.dev/api/objects -> https://svr.nl/api/objects
    let targetPath = url.pathname
    if (url.search) targetPath += url.search
    
    // Prevent double slashes if any
    const targetUrl = `https://svr.nl${targetPath}`
    ...
    proxyHeaders.set('Origin', 'https://svr.nl')
    proxyHeaders.set('Referer', 'https://svr.nl/')
    ...
    if (svrResponse.status === 301 || svrResponse.status === 302) {
      console.warn(`[${requestId}] SVR returned a redirect (${svrResponse.status}). Not following, returning as error.`);
      return new Response(JSON.stringify({ 
        error: "SVR Redirect Detected", 
        details: `SVR returned status ${svrResponse.status}. Location: ${svrResponse.headers.get('Location')}`,
        location: svrResponse.headers.get('Location')
      }), {
        status: 400, // Or 302, depending on desired PWA behavior
